...
- **MySQL** v8.0 and **Redis** v7 (for non-Docker setup)
...
3. **Configure Environment Variables**:
   ...
   ```env
   # Database
   DATABASE_URL="mysql://username:password@localhost:3306/ss_ecommerce"
...
2. **Configure MySQL**:
   ...
   GRANT ALL PRIVILEGES ON ss_ecommerce.* TO 'your_username'@'localhost';
   FLUSH PRIVILEGES;
   EXIT;
   ```
...
2. **Database Setup**:
   ...
   - Use managed MySQL (e.g., PlanetScale, Hostinger, AWS RDS)
...
DATABASE_URL=mysql://localhost:3306/ss_ecommerce_dev
```
...
```env
NODE_ENV=production
DATABASE_URL=mysql://user:pass@prod-db-host:3306/ss_ecommerce_prod
```
...
# Check connection
mysql -h localhost -u your_username -p ss_ecommerce
