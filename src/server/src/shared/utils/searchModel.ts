  ...
  return await prisma[model].findMany({
    where: {
      OR: fields
        .map(({ name, isString }) => {
          if (isString) {
            // For string fields, use "contains" search
            // Note: Prisma client version compatibility may not expose the 'mode' option in types,
            // so we avoid passing it here to prevent TS errors during startup tests.
            return {
