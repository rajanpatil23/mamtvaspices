...
export class CartService {
  ...
  async mergeCartsOnLogin(sessionId: string, userId: string | undefined) {
    ...
    console.log("🔍 [CART SERVICE] Session cart items:", sessionCart?.cartItems?.length);

    if (!sessionCart || !sessionCart.cartItems || sessionCart.cartItems.length === 0) {
      console.log("🔍 [CART SERVICE] No session cart or empty cart, nothing to merge");
      // Still return user cart even if no merge needed
      const userCart = await this.getOrCreateCart(userId);
      console.log("🔍 [CART SERVICE] Returning existing user cart:", userCart);
      return userCart;
    }

    const userCart = await this.getOrCreateCart(userId);
    console.log("🔍 [CART SERVICE] User cart retrieved:", userCart);
    console.log("🔍 [CART SERVICE] User cart items before merge:", userCart?.cartItems?.length);
    ...
    const mergedCart = await this.getOrCreateCart(userId);
    console.log("🔍 [CART SERVICE] Merged cart fetched:", mergedCart);
    console.log("🔍 [CART SERVICE] Merged cart items count:", mergedCart?.cartItems?.length);
    console.log("🔍 [CART SERVICE] Merged cart items:", mergedCart?.cartItems);
